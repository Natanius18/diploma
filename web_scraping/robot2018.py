...
def scrap_page():
    global button_num, uni_name, spec_num, button_num_start
...
while True:
    try:
        ...
        url = 'https://vstup2018.edbo.gov.ua/offers/?qualification=1&education-base=40&speciality=011&course=1&region=51'
        ...
        for spec_num in range(spec_num_start, len(specialities)):
            ...
            for city_num in range(city_num_start, len(cities)):
                choose_city.click()
                sleep(0.1)
                input_city = browser.find_element(By.CLASS_NAME, 'select2-search__field')
                input_city.send_keys(cities[city_num])
                # print(cities[city_num])
                sleep(0.1)
                input_city.send_keys(Keys.ENTER)
                sleep(0.1)
                browser.find_element(By.ID, 'button-offers-search').click()
                ...
                if browser.find_elements(By.CLASS_NAME, 'ui-dialog-buttonset'):
                    ...
                    sleep(8)
                    continue
                # print('spec_num_start:', spec_num, '/', len(specialities))
                # print('city_num_start:', city_num, '/', len(cities))
                ws["O1"] = spec_num
                ws["P1"] = city_num
                wb.save(filename)
